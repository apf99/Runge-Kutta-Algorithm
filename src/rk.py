...
class RK4(object):
    ...
    def _solve(self, y, t, h):
        ...
        k2 = []
        for f in functions:
            k2.append(h * f(t + .5*h, *[y[i] + .5*k1[i] for i in xrange(0, len(y))]))

        k3 = []
        for f in functions:
            k3.append(h * f(t + .5*h, *[y[i] + .5*k2[i] for i in xrange(0, len(y))]))

        k4 = []
        for f in functions:
            k4.append(h * f(t + h, *[y[i] + k3[i] for i in xrange(0, len(y))]))
